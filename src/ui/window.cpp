...
static UI_Movable_Rect_S* currently_moved_rectangle =
  nullptr; // Currently dragged rectangle.
  ...
  if(button == GLFW_MOUSE_BUTTON_LEFT) {
    if(action == GLFW_PRESS) {
      ...
      for(auto& mr: movable_rectangles) {
        if(movable_rect::is_under_mouse(mr, xpos, ypos)) {
...
static void cursor_position_callbacks(GLFWwindow* win, double xpos, double ypos)
{
  ...
  if(currently_moved_rectangle != nullptr) {
    ...
    movable_rect::move(currently_moved_rectangle, x_offset, y_offset);
    last_x = x;
    last_y = y;
  }
}
...
// Any other function can be passed to the constructor as long as it matches
// the pattern:
// return -> type void
// Single parameter -> type Rect_f32
static void render(anton::Rect_f32 position)
{
  GLfloat vertices[] = {position.left,   position.bottom, position.right,
...
namespace ui_window {
  u8 init()
  {
    ...
    movable_rectangles.emplace_front(
      movable_rect::init(&render, width, height));
    movable_rectangles.emplace_front(
      movable_rect::init(&render, width, height));
